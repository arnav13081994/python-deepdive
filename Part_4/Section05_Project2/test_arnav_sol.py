...
8) 6) and 7) for sub, mul, pow both in place as well as regular operations
9) Expected Behaviour for lt, lte and all other operations.
...
def test_iadd_happy():
	...
	Please note that since we are testing the not in place operator before this, here we are only making sure
	that the returned object has the same memory address. The actual operation is not being checked.'''
...
def test_pow_happy():
	...
	# Addition of 1 mod object with an int
	assert Mod(10, 10) ** 0 == Mod(1, 10)
	assert Mod(8, 4) ** 11 == Mod(4, 4)
	assert Mod(8, 3) ** 10 == Mod(2, 3)
	...
	# Success
	assert Mod(-115, 20) ** Mod(15, 20) == Mod(5, 20)
	assert Mod(11, 3) ** Mod(8, 3) == Mod(1, 3)

	# Failure
	assert Mod(110, 20) ** Mod(-33, 20) != Mod(1, 20)
	assert Mod(-115, 20) ** Mod(-25, 20) != Mod(21, 20)
def test_ipow_happy():
	''' Tests in place subtraction operators which needs to return the same hex(id(Mod)).
	Please note that since we are testing the not in place operator before this, here we are only making sure
	that the returned object has the same memory address. The actual operation is not being checked.'''
...
# 'lt' test cases
def test_lt_happy():
	''' Tests the less than operator. Because total_ordering is being used and __eq__ has already been tested.
	Testing < operator alone would be sufficient to test all other operations'''
	...
	# Success
	assert (Mod(-115, 20) < Mod(27, 20))
	assert (Mod(9, 3) < Mod(8, 3))
